...
`New key` button creates a new key for default server and sends it to user. User's @username in that case will be used as a name of the key. For more details about the default server settings, see [adding servers](#adding-servers).
...
-   `/newkey <server_id> <key_name>`, where `server_id` is internal unique id of each server, `key_name` is a string with a name of the key. Use `/servers` to see available server ids.
